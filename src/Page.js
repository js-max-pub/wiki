...
export default class WikiPage {
	...
	async box(boxName, options = {}) {
		...
		let output = boxFunction(text,boxName,options)	
		return output
	}

}
function boxFunction(text,boxName,options ={}){
	...
	for (let line of lines) {
		if (line.startsWith('|')) { // lines with a key
			...
			if (val.startsWith('*')){ // multi-line-entry
				...
				brackets = isParsingBrackets? parseBrackets(val): [val]
				output[key] = [...brackets]
			}
			else {
				brackets = isParsingBrackets? parseBrackets(val): [val]
